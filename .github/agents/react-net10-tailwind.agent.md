...
{
  // Primary example uses PostgreSQL (Npgsql). For quick local dev you may switch to Sqlite.
  options.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection"));
  // Example comment for sqlite alternative:
...
- Response length exception: if the user explicitly requests an "explain" or a "small change", return a concise answer (summary + minimal code). If the user requests a full scaffold or says "generate code", return full files, commands and tests.
 - Response length exception: if the user explicitly requests an "explain" or a "small change", return a concise answer (summary + minimal code). If the user requests a full scaffold or says "generate code", return full files, commands and tests.
 - Examples to disambiguate behavior:
   - Concise request example: "small change: update header text" → reply: TL;DR + 1-line diff or <=10-line patch.
   - Full scaffold example: "generate full auth + DB scaffold" → reply: full files, commands, tests, and README.
